...
namespace daw::burp::benchmark {
	template<typename Rep, typename Period>
	DAW_ATTRIB_NOINLINE [[nodiscard]] std::string
	ns_to_string( std::chrono::duration<Rep, Period> time, std::size_t prec = 0 ) {
		std::stringstream ss;
		ss << std::setprecision( static_cast<int>( prec ) ) << std::fixed;
		using dsec_t = std::chrono::duration<double, std::ratio<1>>;
		auto val = std::chrono::duration_cast<dsec_t>( time ).count( ) * 1'000'000'000'000'000.0;
		if( val < 1000.0 ) {
			ss << val << "fs";
			return ss.str( );
		}
	...
	DAW_ATTRIB_NOINLINE std::string to_min_SI_unit_full( Bytes bytes, std::size_t prec = 1 ) {
		std::stringstream ss;
		ss << std::setprecision( static_cast<int>( prec ) ) << std::fixed;
	...
	template<typename Bytes>
	DAW_ATTRIB_NOINLINE std::string to_min_SI_unit( Bytes bytes, std::size_t prec = 1 ) {
		std::stringstream ss;
		ss << std::setprecision( static_cast<int>( prec ) ) << std::fixed;
		auto val = static_cast<double>( bytes );
		if( val < 1000.0 ) {
	...
	daw::expected_t<std::invoke_result_t<Func, Args...>> benchmark( std::size_t min_num_runs,
		...
		using dsec_t = std::chrono::duration<double, std::ratio<1>>;
		auto const max_per_second =
		  static_cast<std::uint64_t>( static_cast<double>( min_num_runs ) /
		                              std::chrono::duration_cast<dsec_t>( min_duration ).count( ) );
		auto const max_throughput = static_cast<double>( data_size ) /
		                            std::chrono::duration_cast<dsec_t>( min_duration ).count( );
		auto const min_throughput = static_cast<double>( data_size ) /
		                            std::chrono::duration_cast<dsec_t>( max_duration ).count( );
		auto const min_per_second =
		  static_cast<std::uint64_t>( static_cast<double>( min_num_runs ) /
		                              std::chrono::duration_cast<dsec_t>( max_duration ).count( ) );
		auto const avg_throughput = static_cast<double>( data_size ) /
		                            std::chrono::duration_cast<dsec_t>( avg_duration ).count( );
		auto const avg_per_second =
		  static_cast<std::uint64_t>( static_cast<double>( min_num_runs ) /
		                              std::chrono::duration_cast<dsec_t>( avg_duration ).count( ) );
